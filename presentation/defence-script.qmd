...
The mechanism is the reference frame. Classic TOPSIS normalises each criterion by the sample's own magnitude, so SVC-04's four hundred and ten incidents dominate the incident axis and pull it fractionally ahead. VIKOR's regret term caps on SVC-01's worst-in-portfolio values, the lowest patch recency at sixty-five point eight and four thousand and fifty-four unsupported component months, and the absolute variant, measuring against fixed bounds, registers that same structural exposure at full strength. So the split separates methods that register concentrated structural exposure fully from sample-relative distance aggregation. The framework reports the margin rather than hiding it, and in 2025, and under 2024 adjusted weights, every method puts SVC-01 first.
