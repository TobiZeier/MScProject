...
The stability results. Between the two discrete weight configurations, agreement in 2025 is perfect: tau equals one, the identical order under both weightings. In 2024 it is nought point six, as reweighting reorders the middle of the field. Across years, tau is nought point six under equal weights and nought point four six seven under adjusted, and that movement tracks real change: SVC-01, well maintained in 2024, moved to first in 2025 as its patch recency fell and its unsupported-component exposure grew, and SVC-06 also climbed as it deteriorated broadly.
...
This slide carries the finding I did not expect. In 2025 the methods agree exactly under equal weights, all placing SVC-01 first, and SVC-03 is last everywhere. The interesting case is 2024, where the methods agree on the two highest-priority services but split on which of them leads.

All four scorers name SVC-04 and SVC-06 as the high-priority pair. Classic TOPSIS puts SVC-04 first; SAW and VIKOR put SVC-06 first, with SVC-04 close behind in both.

Why do they split? The two services are extreme in different ways. SVC-04 has by far the highest incident count, five hundred and seventy-four, and vector-normalised distance preserves that raw magnitude, so TOPSIS ranks it first. SVC-06 carries the portfolio's worst change failure rate, and both a linear sum and VIKOR's regret term weight that heavily, so those two methods rank it first instead. Beneath the pair the methods realign completely: all of them place SVC-02 and SVC-01 at ranks three and four, separated by only five thousandths. So the disagreement is not noise. It sits exactly where the scores are closest, and it names the two services a manager should look at first regardless of method.
