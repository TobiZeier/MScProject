...
No, for three reasons, though churn does make the complementary role more important. First, re-ranking after churn is correct behaviour, not rank reversal: prioritisation is a competition, and a competition with different entrants legitimately has a different result; the pathology is unmotivated flips among unchanged services, and the leave-one-out test bounds that at single adjacent swaps that never cross the portfolio halves. Second, absolute mode is only churn-proof while its fixed bounds hold; a new arrival outside the calibrated bounds forces either clipping, which destroys discrimination where the signal is strongest, or bound revision, which shifts every score at once and readmits instability as a governance problem. Third, churn makes spiky new profiles more likely, which is exactly the shape the compensatory absolute variant masks, as SVC-06 in 2024 shows. In deployment the division of labour therefore holds: sample-relative methods answer who is first in today's portfolio, while the absolute frame tracks whether an individual service is deteriorating on a scale that does not move (Chapter 5).
