...
Reads a CSV of services, scores them with equal and adjusted weights,
prints both ranked tables, and saves two bar charts.
"""
...
EQUAL_WEIGHTS = np.array([0.20, 0.20, 0.20, 0.20, 0.20], dtype=float)
ADJUSTED_WEIGHTS = np.array([0.13333, 0.30, 0.13333, 0.3, 0.13333], dtype=float)

OUTPUT_CHART_EQUAL = "topsis_ranking_equal.png"
OUTPUT_CHART_ADJUSTED = "topsis_ranking_adjusted.png"
...
def main():
    ...
    result_equal = run_topsis(df, CRITERIA, COST_CRITERIA, EQUAL_WEIGHTS)
    result_adjusted = run_topsis(df, CRITERIA, COST_CRITERIA, ADJUSTED_WEIGHTS)

    print_results(result_equal, "TOPSIS Results - Equal Weights (0.20 per criterion)")
    print_results(
        result_adjusted,
        "TOPSIS Results - Adjusted Weights (0.13333, 0.30, 0.30, 0.13333, 0.13333)"
    )

    save_chart(
        result_equal,
        OUTPUT_CHART_EQUAL,
        "Equal weights: 0.20 per criterion | Red = Rank 1 (highest remediation priority)"
    )
    save_chart(
        result_adjusted,
        OUTPUT_CHART_ADJUSTED,
        "Adjusted weights: 0.13333, 0.30, 0.30, 0.13333, 0.13333 | Red = Rank 1"
    )
