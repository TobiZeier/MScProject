...
{\normalsize 29 July 2026}\\[1cm] % Date
...
and operational instability, where the dominant code-centric measurement
tools cannot observe it. This thesis develops and evaluates a
quantitative framework for prioritising technical debt at the service
...
Swiss financial trading institution and combined in a TOPSIS model that
ranks six production services by debt priority under two weight
configurations and across two observation years. The evaluation tests
the ranking's stability under weight and temporal variation, its
convergence with SAW and VIKOR, its divergence from an incident-only
baseline, and its structural rigour under service removal, the last
addressed through a rank-reversal-free absolute-mode TOPSIS variant.
Scored with that variant across 20,000 sampled weightings, the
lowest-priority service is near-invariant to the weighting, while the
single highest priority is weight-dependent within a stable
three-service cluster; the multi-criteria design surfaces infrastructure
lifecycle debt that an incident count misses; and method disagreement
localises to that high-priority cluster, exposing a substantive choice
about whether debt dimensions are compensatory. The contribution is, to
the best of current knowledge, the first application of TOPSIS to live
service management records for technical debt prioritisation in
financial trading IT, together with an auditable evaluation logic suited
to regulated environments. The rankings are ordinal decision-support
signals rather than measurements of debt, and external and longitudinal
validation are identified as the principal directions for future work.
...
a small number of years (Banker, Liang and Ramasubbu, 2021). In trading
environments they are intensified by continuous availability
requirements, vendor-dominated architectures and regulatory oversight
(\emph{{FINMA Circular} 2023/1 - {Operational} risks and resilience --
banks}, 2024), making the absence of a quantitative prioritisation
...
as a testable hypothesis. None of the literature reviewed applied a
multi-criteria decision-making framework to live service management data
to produce a ranked technical debt prioritisation instrument for
financial trading IT. This specific absence motivates the framework
developed here.
...
gap. Chapter~\ref{sec-chapter3} describes the research design,
methodology and ethical considerations. Chapter~\ref{sec-chapter4}
presents the framework development and results, including data
preparation and TOPSIS computation. Chapter~\ref{sec-chapter5} evaluates
the framework against the criteria established in
Chapter~\ref{sec-chapter3}, including stability, convergent and
...
unavailable, even though in such contexts debt manifests in operational
...
financial trading IT, this broader view is essential. Vendor platforms
and COTS products form the core of the operational stack, and platform,
infrastructure and vulnerability debt are not detectable through
standard static analysis. Managing them requires indicators derived from
infrastructure records and operational monitoring rather than from code
repositories.
...
rates. These are precisely the signals available where source code is
inaccessible, and they establish that a quantitative framework must keep
its value judgements transparent and auditable in regulated
institutions.
...
as a general empirical law, and the framework treats it as a hypothesis
...
monitoring. The public sector context limits transferability, but the
operational mechanism matches the vendor-dominated profile of the
present case.
...
transparent, though its linear aggregation is compensatory, offsetting a
weak score on one criterion against a strong score on another in
proportion to their weights. A risk matrix, the other common baseline,
is categorical and cannot finely separate a large service portfolio.
TOPSIS and SAW are both compensatory, yet TOPSIS scores each alternative
by its distance to both an ideal best and an ideal worst rather than by
a single weighted sum. This is a difference of aggregation structure
rather than guaranteed outcome. Ciardiello and Genovese (2023) show that
under some distance metrics TOPSIS reproduces SAW closely. Method choice
is therefore treated here as a robustness question, examined in
Chapter~\ref{sec-chapter5}, rather than as a matter of correctness.
...
evaluation approach, and the second addresses the ethical and
professional considerations arising from the use of operational data in
...
(Section~\ref{sec-frameworkdesign}); evaluation uses established
...
The process follows Peffers \emph{et al.} (2007) six-step model: problem
identification and motivation (Chapter~\ref{sec-chapter1} and
...
outsourced components. Raw vulnerability density was excluded as
subsumed by patch recency, a more stable CMDB-derived proxy grounded in
regulatory patching compliance rather than scanner counts whose coverage
...
framework targets, since code-level tools disagree on the same codebase
...
Biazotto \emph{et al.}, 2024), while the agreed governance scope
...
window means the framework captures a snapshot, so recently remediated
...
Two years of live ITSM and CMDB records (January 2024 to December 2025)
from a regulated Swiss trading institution supply the raw material for
this chapter. The indicator values extracted from those records, the
normalisation and weighting decisions applied to them, and the TOPSIS
scores that result are each documented and justified in the sections
below.
...
\bottomrule\noalign{}
\endlastfoot
SVC-04 & 0.4671 & 1 & 0.4927 & 2 \\
SVC-06 & 0.5339 & 2 & 0.6210 & 4 \\
SVC-02 & 0.6796 & 3 & 0.4898 & 1 \\
SVC-01 & 0.6843 & 4 & 0.5660 & 3 \\
SVC-05 & 0.6964 & 5 & 0.7032 & 5 \\
SVC-03 & 0.8450 & 6 & 0.8796 & 6 \\
\end{longtable}

\endgroup

Two year-on-year changes are substantial. SVC-01 shifted the most: in
2024 it was well maintained, with patch recency of 98.23 and only 108
unsupported component-months, but by 2025 its patch recency had fallen
to 53.10 and its unsupported exposure had risen to 2,565
component-months, the largest in the portfolio. SVC-06 also deteriorated
broadly, its incident count rising from 43 to 302, its change failure
rate from 0.86 to 3.96, and its patch recency falling from 82.35 to
52.94. SVC-02's unsupported-component exposure rose from 34 to 712
component-months, drawn from the same complete CMDB extract that
underlies the indicator (Appendix~\ref{sec-appendixb}).
...
removal of individual services. A final section reads the rankings
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1500}}@{}}
\caption{Kendall's tau across ranking comparisons, standard
...
Weight sensitivity in 2025 is perfect (\(\tau = 1.000\)), with both
configurations producing the identical order. In 2024 it is moderate
(\(\tau = 0.600\)), as reweighting MTTR and patch recency reorders the
middle of the field. Temporal stability is also moderate
(\(\tau = 0.600\) equal, \(0.467\) adjusted), and the movement follows
real profile change. SVC-01 is the clearest case. Well maintained in
2024, it ranked fourth on debt priority; by 2025 its patch recency had
fallen to 53.10 and its unsupported exposure had risen to the
portfolio's largest, moving it to first. SVC-06 also climbed as its
incident load, change failure rate and patch position worsened.
Cross-year movement is reported as descriptive context, not as a quality
criterion in its own right. Services evolve between periods, and a
faithful ranking should follow that change. In every run the
lowest-priority position is identical, and the same three services
occupy the high-priority half in 2025 under both configurations, which
addresses RQ3 directly.
...
TOPSIS. SAW used Weitendorf (min-max) normalisation to avoid the
division by zero that classic cost normalisation would produce on the
dataset's zero values (Thien, Dung and Trung, 2024). VIKOR followed the
standard S, R and Q procedure with \(v = 0.5\). Full figures are in
Appendix~\ref{sec-appendixc}.

Agreement ranges from moderate to perfect (\(\tau\) from 0.600 to
1.000). In 2025 the three methods agree exactly under equal weights, all
placing SVC-01 first, and SVC-03 comes last under every method,
configuration and year. The disagreement concentrates in 2024 and
concerns the identity of the single highest-priority service. TOPSIS
places SVC-04 first, whereas SAW and VIKOR place SVC-06 first, with
SVC-04 second in both. All three methods agree that SVC-04 and SVC-06
form the high-priority pair. They differ only on which of the two leads.
The reason is the aggregation structure. SVC‑06 carries the portfolio's
worst patch recency (82.35) and its highest unsupported‑component
exposure (279 months), infrastructure lifecycle weaknesses that VIKOR's
regret term and SAW's linear sum weight heavily. SVC‑04 carries by far
the highest incident count, whose magnitude vector‑normalised distance
preserves and the other two methods compress. Beneath that pair TOPSIS
and VIKOR realign on the same order, both placing SVC‑02 third and
SVC‑01 fourth, separated in the TOPSIS scores by only 0.0047. SAW
diverges here, ranking SVC‑01 third and SVC‑02 fifth. The broad
prioritisation is therefore method-independent, since every method
identifies the same high-priority pair and the same lowest-ranked
service, while the residual disagreement sits on the internal order of
closely scored services, a point revisited in
...
by incident frequency alone, the orderings diverge (\(\tau = 0.467\) in
2025, \(0.067\) in 2024; the full comparison is in
Appendix~\ref{sec-appendixc}). The divergence concentrates in two
services. SVC-01 rises from fourth on incident count to first overall,
carrying 2,565 unsupported component months and a patch recency of 53.10
that an incident count does not register. SVC-05 falls from third to
fifth for the mirror-image reason, as its incident volume is offset by
full patch recency and no unsupported components. The result is
contingent. Indicators that moved together would have reproduced the
incident ranking however many were added. They carry distinct
information instead, which is the condition under which the
multi-criteria design adds value (Albarak \emph{et al.}, 2022). What the
comparison does not establish is that the composite ordering is the more
correct one. That rests on the indicator-to-dimension mapping inherited
...
re-ranking the remainder. Under the standard formulation the order holds
in four of six removals (Table~\ref{tbl-rankreversal}). The other two
reach the top of the ranking, swapping SVC-01 and SVC-04 or lifting
SVC-06 into first place. The lowest-priority service never moves, but
the identity of the top-priority service depends on which other services
are present. This is the limitation of set-relative distance aggregation
identified by García-Cascales and Lamata (2012), and in this data it is
...
SVC-04 & SVC-06 moves above SVC-01 into rank 1; SVC-05 and SVC-02
exchange \\
...
and Lamata (2012) was implemented. Vector normalisation is replaced by
normalisation against fixed criterion bounds, and the ideal solutions
are anchored direction-aware to fictitious alternatives at the extremes
of each criterion scale, addressing the cost-attribute limitation that
Yang (2020) identify in the original formulation. Under this variant the
...
(2020) prove that variants with set-independent bounds and ideals are
...
2025, with agreement against SAW and VIKOR at \(\tau = 0.867\) or higher
in every configuration, its only deviation placing SVC-06 above SVC-04
at ranks 2 and 3. Its distinctive behaviour appears in 2024, where
fixing the reference frame changes the order more sharply. Whereas the
standard formulation ranks SVC-04 first and SVC-06 second, the absolute
variant ranks SVC-04 first but demotes SVC-06 to fifth, because the
fixed bounds no longer let SVC-06's sample-relative extremity dominate.
The reason is the reference frame described in
Section~\ref{sec-convergent}. Measured against fixed bounds rather than
the sample's own range, a service that is merely worst-in-sample on a
criterion no longer counts as extreme unless it approaches the declared
scale limit.

The variant's year-on-year concordance (\(\tau = 0.333\) under both
configurations) follows from the same genuine profile movements and, per
Section~\ref{sec-stability}, is not read as a deficiency. The absolute
frame retains two properties no other method offers here: its rankings
survive every leave-one-out removal unchanged, where the standard
formulation's top rank flips, and its scores remain comparable across
periods, because the other methods rescale against whichever services
are in that year's sample.
...
signal, and the 2024 result shows why triangulation is reported rather
than a single number. All four scorers agree that SVC-04 and SVC-06 are
the two highest-priority services, but they divide on which leads and on
how far SVC-06 falls once the reference frame is fixed. The
absolute-mode variant additionally contributes rank-reversal immunity
and cross-period comparability, certifying that the primary ranking is
not an artefact of sample composition.
...
SVC-03 at the lowest priority in 98.1\% of weightings. The top is a
property of the cluster rather than a single service: rank 1 falls to
SVC-01, SVC-06 or SVC-04 in every single draw, split 45.6\%, 27.1\% and
27.3\% respectively. The extreme low priority is thus robust to any
weighting, while the single top service is weight-dependent within a
stable three-service cluster, consistent with
Section~\ref{sec-absolutemode}, which addresses RQ3.
...
in 71.3\% of weightings rather than 98.1\%, and the top rank spreads
across three services, with SVC-04 taking it in 58.3\% of draws, SVC-02
in 33.8\% and SVC-06 in 7.9\%. This mirrors the weaker 2024 figures in
...
Three services illustrate what the rankings capture. SVC-01 holds rank 1
under both configurations, pairing a modest incident count (64 per year,
fourth of six) with the portfolio's highest MTTR (45.35 hours), a patch
recency of 53.10 and 2,565 unsupported component months, by far the
largest exposure recorded. Table~\ref{tbl-indicators-lit} places this
debt in the infrastructure lifecycle, vulnerability and
infrastructure/platform dimensions (Nord, Ozkaya and Woody, 2021; Haki
\emph{et al.}, 2023). This is the signal the framework is built to
surface, a service that looks quiet on incident metrics while carrying
dominant structural debt. SVC-04 holds rank 2 with the mirror-image
profile: the highest incident frequency (574 per year), an MTTR of 41.91
hours and a CFR of 3.38, yet no unsupported components and full patch
recency, the indicators of architectural and process debt (de Toledo
\emph{et al.}, 2021; Nord, Ozkaya and Woody, 2021). SVC-03 has the
lowest priority in every run and is the reference case for a
well-maintained service, with the fewest incidents, low MTTR and CFR,
full patch recency and no unsupported components. The one point where
the methods disagree, the 2024 order within the SVC-04 and SVC-06
high-priority pair, is examined in Section~\ref{sec-absolutemode}.

These results address RQ2. A cross-sectional design cannot establish
whether the observed metrics are downstream effects of technical debt
severity or arise from other operational factors, so proxy validity is
assessed on the basis of convergence with theory rather than
demonstrated causal mechanism. Each profile corresponds to a debt
pattern the literature documents (Ramasubbu and Kemerer, 2021; Nord,
Ozkaya and Woody, 2021; Haki \emph{et al.}, 2023), and the profiles are
distinct from one another rather than expressions of a single underlying
deficiency, which is what makes the ordering readable against
Table~\ref{tbl-indicators-lit}. That correspondence is a consistency
check rather than an independent test, since the ranking is computed
from the same indicators that constitute each profile.
...
method disagreement in Section~\ref{sec-convergent} and
Section~\ref{sec-absolutemode} adds a further boundary. The framework
supports cluster-level confidence, but the single top rank is contested
within the SVC-01, SVC-04 and SVC-06 cluster across the weight space,
turning on whether concentrated structural exposure is judged more
urgent than diffuse operational debt, a compensability judgement the
framework informs rather than settles. Finally, the cross-sectional
design captures a single snapshot, so a service mid-remediation may look
worse than warranted. Longitudinal evaluation is flagged in
Chapter~\ref{sec-chapter6}, and the data quality limitations in
Section~\ref{sec-datalimitations} apply here, with middle-position
rankings warranting careful reading.
...
that the ordering diverges substantially from an incident-only view in
the current data (\(\tau = 0.467\) in 2025, and more weakly, \(0.067\),
in 2024). That divergence was not guaranteed. Indicators that moved
together would have reproduced the incident ranking and made TOPSIS
redundant. SVC-01 rises from fourth to first, carrying 2,565 unsupported
component months an incident count does not register, and SVC-05 falls
because its incident volume comes with a clean infrastructure profile.
...
underlying deficiency: SVC-01 carries dominant structural exposure
behind a quiet incident record, SVC-04 the reverse, and SVC-03 neither.
Indicators that moved together would have made the multi-criteria design
redundant, and the discriminant test confirms that no single indicator
reproduces the ordering. This is a necessary condition for the proxy
claim, not a sufficient one. Criterion validity cannot be established,
because no independent measure of non-code debt exists to validate
against. The proxy relationship therefore keeps the status given in
...
perturbation. They are stable at the extremes and contested at the very
top. The two discrete configurations agree perfectly in 2025
(\(\tau = 1.000\)). The Monte Carlo extension across 20,000 sampled
weightings places SVC-03 last in 98.1\% of them, and rank 1 falls to
SVC-01, SVC-06 or SVC-04 in every draw. The single top service is
therefore weight-dependent within a stable cluster. In 2024 the methods
agree on the high-priority pair but split on its internal order, TOPSIS
leading with SVC-04 while SAW and VIKOR lead with SVC-06. As
Section~\ref{sec-absolutemode} showed, that disagreement reflects how
differently methods treat a service that is extreme relative to the
sample but not against fixed bounds, and it marks the boundary between
...
\NormalTok{ABSOLUTE\_BOUNDS }\OperatorTok{=}\NormalTok{ \{}
    ...
    \StringTok{"unsupported\_months"}\NormalTok{: (}\FloatTok{0.0}\NormalTok{, }\FloatTok{3000.0}\NormalTok{)}
\NormalTok{\}}
...
Filesize: \texttt{251\ bytes}

\begin{verbatim}
service,incident_freq,mttr_hrs,cfr,patch_recency,unsupported_months
SVC-01,65,55.37,0.29,98.23,108
SVC-02,39,81.35,0.11,99.66,34
SVC-03,63,9.77,0.86,100.00,0
SVC-04,410,41.53,2.19,100.00,0
SVC-05,140,30.30,1.41,100.00,0
SVC-06,43,15.87,0.86,82.35,279
\end{verbatim}
...
VIKOR is \(\tau = 0.867\): the variant places SVC-06 above SVC-04 at
ranks 2 and 3, where the standard formulation and both alternative
methods place SVC-04 second. This reflects the effect of the fixed
reference frame on the closely scored cluster below SVC-01 and
accompanies the rank-reversal results in
Table~\ref{tbl-appc-rankreversal-abs}.
...
\endlastfoot
2025 &
\(\text{SVC-01} \succ \text{SVC-04} \succ \text{SVC-06} \succ \text{SVC-02} \succ \text{SVC-05} \succ \text{SVC-03}\)
&
...
SVC-04 &
\(\text{SVC-06} \succ \text{SVC-01} \succ \text{SVC-05} \succ \text{SVC-02} \succ \text{SVC-03}\)
&
\(\text{SVC-06} \succ \text{SVC-05} \succ \text{SVC-01} \succ \text{SVC-02} \succ \text{SVC-03}\) \\
SVC-05 & preserved & preserved \\
SVC-06 &
\(\text{SVC-04} \succ \text{SVC-01} \succ \text{SVC-05} \succ \text{SVC-02} \succ \text{SVC-03}\)
&
...
\(\text{SVC-01} \succ \text{SVC-06} \succ \text{SVC-04} \succ \text{SVC-02} \succ \text{SVC-05} \succ \text{SVC-03}\);
2024
\(\text{SVC-04} \succ \text{SVC-02} \succ \text{SVC-01} \succ \text{SVC-05} \succ \text{SVC-06} \succ \text{SVC-03}\).
...
\(\text{SVC-01} \succ \text{SVC-04} \succ \text{SVC-06} \succ \text{SVC-02} \succ \text{SVC-05} \succ \text{SVC-03}\)
&
\(\text{SVC-02} \succ \text{SVC-04} \succ \text{SVC-01} \succ \text{SVC-06} \succ \text{SVC-05} \succ \text{SVC-03}\)
& 0.467 \\
\end{longtable}
