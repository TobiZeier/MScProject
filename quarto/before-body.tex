...
$if(book.subtitle)$
{\normalsize\bfseries $book.subtitle$\par}\vspace{0.2cm} %Thesis subtitle
$endif$
...
\emph{Author:}\\
$if(author.url)$
\href{$author.url$}{\authorname}\\[0.3cm] % Author name - remove the \href bracket to remove the link
\emph{Student ID:}\\
\thesistudentid
$else$
\authorname\\[0.3cm]
...
\noindent $thesis.declaration.text$
$else$
\noindent\input{"$thesis.declaration$"}
$endif$
...
\noindent $thesis.acknowledgements.text$
$else$
\noindent\input{"$thesis.acknowledgements$"}
$endif$
