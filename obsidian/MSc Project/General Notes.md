...
- [ ] end literature review with a table (supervisor suggestion)
...
- [ ] specification for data usage:
	- [ ] patch recency: percentage of components that have been flagged to end of support,  not end of life, either by the vendor or the company itself
	- [ ] mention that only assets in production were taken into account
	- [ ] excluded: issues that have raised several incidents but same root cause (i.e. an alarm triggered the recurring creation of an incident)
...
- [ ] finish chapter 4
	- [ ] if materially incomplete CMDB records exist, flag them in section  data preparation, if not, remove remark in section data limitation
	- [x] remove equations from chapter 4
	- [ ] Rewrite **Data Preparation** section, using actual numbers.  Inclusion and exclusion criteria - justify which services from the trading IT portfolio have been excluded and why
	- [ ] Rewrite **Indicator Extraction and Quality Checks** use actual numbers and checks
